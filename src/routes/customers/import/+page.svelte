<!-- src/routes/customers/import/+page.svelte (ปรับปรุงสำหรับ Svelte 5) -->
<script lang="ts">
	import { enhance } from '$app/forms';
	import type { ActionData } from './$types';

	// [แก้ไข] เปลี่ยนมาใช้ $props() ตามมาตรฐาน Svelte 5
	let { form } = $props<ActionData>();

	// [ปรับปรุง] ใช้ $state() สำหรับจัดการค่าที่เปลี่ยนแปลงบน UI
	let isLoading = $state(false);
</script>

<svelte:head>
	<title>นำเข้าข้อมูลสมาชิก</title>
</svelte:head>
...
<main class="container" style="max-width: 768px;">
	...
	<article>
		...
		<ol>
			<li>
				ดาวน์โหลดเทมเพลตไฟล์ Excel <a href="/template-customers.xlsx" download>ที่นี่</a>
			</li>
			<li>กรอกข้อมูลสมาชิก (ช่องที่มีคำว่า "บังคับ" ห้ามเว้นว่าง)</li>
			<li>เลือกไฟล์ Excel ของคุณและกดปุ่ม "นำเข้าข้อมูล"</li>
		</ol>
		<p><strong>ข้อควรระวัง:</strong> สมาชิกที่มี "รหัสสมาชิก" ซ้ำกับที่มีอยู่แล้วในระบบจะถูกข้ามไป</p>
	</article>
	...
	<form
		...
		use:enhance={() => {
			isLoading = true;
			form = undefined; // เคลียร์ข้อความเก่าเมื่อเริ่มส่งข้อมูลใหม่
			return async ({ update }) => {
				await update();
				isLoading = false;
			};
